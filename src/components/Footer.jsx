    ...
    <footer className="w-full py-section-gap bg-surface-container-low border-t border-outline-variant">
      <div className="max-w-[1200px] mx-auto px-margin-desktop grid grid-cols-1 md:grid-cols-4 gap-gutter">
        ...
        <div className="flex flex-col gap-6 mr-10">
          ...
          <div className="flex gap-4">
            <Link to='/' className=" text-secondary cursor-pointer hover:text-primary transition-colors">
        ...
        <div className="flex flex-col gap-4">
          ...
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="text-on-tertiary-fixed-variant hover:text-primary font-medium font-sans
              hover:translate-x-1 transition-all duration-200 w-fit"
            >
              {link.linkName}
            </Link>
          ))}
        ...
        <div className="flex flex-col gap-4">
          ...
          {resourceLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="text-on-tertiary-fixed-variant font-medium font-sans  hover:text-primary
               hover:translate-x-1 transition-all duration-200 w-fit"
            >
              {link.name}
            </Link>
        ...
        <button
          type="submit"
          className="absolute right-0 top-0 bottom-0 w-28 bg-primary rounded-r-lg cursor-pointer text-on-primary font-bold text-[12px] tracking-wider 
      ...
      <div className="max-w-[1200px] mx-auto px-margin-desktop mt-20 pt-8 border-t border-outline-variant/30 flex justify-between items-center">
        <span className="text-on-tertiary-fixed-variant font-body-md text-[12px] opacity-70">
          © {new Date().getFullYear()} The Style Parlor. All Right Reserved.
        </span>
